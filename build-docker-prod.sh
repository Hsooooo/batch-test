...
-d 'payload={"text":"@channel [Factsheet] ('${APP_NAME}') 배포 시작"}'
...
docker run -d -p ${PORT}:${PORT} \
    ...
    -e "spring.datasource.url=${FS_DB_URL}" \
    -e "spring.datasource.username=${FS_DB_USER}" \
    -e "spring.datasource.password=${FS_DB_PASSWORD}" \
...
curl -X POST 'https://illunex.synology.me:52582/webapi/entry.cgi?api=SYNO.Chat.External&method=incoming&version=2&token='${SYNOLOGY_HOOK_TOKEN} \
-H 'Content-Type: application/x-www-form-urlencoded' \
-d 'payload={"text":"@channel [factsheet] ('${APP_NAME}') 배포 완료"}'
